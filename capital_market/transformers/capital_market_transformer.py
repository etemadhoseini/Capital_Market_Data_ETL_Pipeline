...
from capital_market.common.meta_process import MetaProcess

class CapitalMarketSourceConfig(NamedTuple):
    """
...
class CapitalMarketTargetConfig(NamedTuple):
    """
...
class CapitalMarketETL():
    """
    Reads the capital market data, transforms and writes the transformed to target
    """

    def __init__(self, s3_bucket_src: S3BucketConnector,
                 s3_bucket_trg: S3BucketConnector, meta_key: str,
                 src_args: CapitalMarketSourceConfig, trg_args: CapitalMarketTargetConfig):
        """
        Constructor for CapitalMarketTransformer
    ...
    def extract(self):
        ...
        """
        self._logger.info('Extracting capital market source files started...')
        ...
        self._logger.info('Extracting capital market source files finished.')
        return data_frame

    def transform_report(self, data_frame: pd.DataFrame):
        """
        ...
        if data_frame.empty:
            ...
            return data_frame
        self._logger.info('Applying transformations to capital market source data for report 1 started...')
        ...
        self._logger.info('Applying transformations to capital market source data finished...')
        return data_frame
    ...
    def load(self, data_frame: pd.DataFrame):
        ...
        self._logger.info('Capital Market target data successfully written.')
        # Updating meta file
        MetaProcess.update_meta_file(self.meta_update_list, self.meta_key, self.s3_bucket_trg)
        self._logger.info('Capital Market meta file successfully updated.')
        return True
    ...
    def etl_report(self):
        ...
        # Transformation
        data_frame = self.transform_report(data_frame)
        # Load
