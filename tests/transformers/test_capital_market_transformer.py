...
class TestCapitalMarketETLMethods(unittest.TestCase):
    """
    Testing the CapitalMarketETL class.
    """
    ...
    def setUp(self):
        ...
        conf_dict_trg = {
            ...
            'trg_key': 'report/daily_report_',
            'trg_key_date_format': '%Y%m%d_%H%M%S',
            'trg_format': 'parquet'
        }
        self.source_config = CapitalMarketSourceConfig(**conf_dict_src)
        self.target_config = CapitalMarketTargetConfig(**conf_dict_trg)
    ...
    def test_extract_no_files(self):
        ...
        return_value=[extract_date, extract_date_list]):
            capital_market_etl = CapitalMarketETL(self.s3_bucket_src, self.s3_bucket_trg,
                         self.meta_key, self.source_config, self.target_config)
            df_return = capital_market_etl.extract()
    ...
    def test_extract_files(self):
        ...
        return_value=[extract_date, extract_date_list]):
            capital_market_etl = CapitalMarketETL(self.s3_bucket_src, self.s3_bucket_trg,
                         self.meta_key, self.source_config, self.target_config)
            df_result = capital_market_etl.extract()
        # Test after method execution
        self.assertTrue(df_exp.equals(df_result))

    def test_transform_report_emptydf(self):
        """
        Tests the transform_report method with
        ...
        return_value=[extract_date, extract_date_list]):
            ...
            with self.assertLogs() as logm:
                df_result = capital_market_etl.transform_report(df_input)
    ...
    def test_transform_report_ok(self):
        ...
        """
        # Expected results
        log1_exp = 'Applying transformations to capital market source data for report 1 started...'
        log2_exp = 'Applying transformations to capital market source data finished...'
    ...
    def test_load(self):
        ...
        """
        # Expected results
        log1_exp = 'Capital Market target data successfully written.'
        log2_exp = 'Capital Market meta file successfully updated.'
        df_exp = self.df_report
        ...
        return_value=[extract_date, extract_date_list]):
            capital_market_etl = CapitalMarketETL(self.s3_bucket_src, self.s3_bucket_trg,
                         self.meta_key, self.source_config, self.target_config)
            with self.assertLogs() as logm:
                capital_market_etl.load(df_input)
                # Log test after method execution
